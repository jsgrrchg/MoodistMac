    ...
    var body: some View {
        List {
            Section {
                VStack(spacing: 12) {
                    HStack(spacing: 12) {
                        SoundSymbol(name: store.displayedMixIconName ?? "waveform")
                            .frame(width: 28, height: 28).foregroundStyle(.tint)
                        Text(store.displayedMixName ?? T("current_mix", "Current mix"))
                            .font(.title2.bold()).fixedSize(horizontal: false, vertical: true)
                        Spacer(minLength: 0)
                        AudioRoutePicker().frame(width: 44, height: 44)
                            .accessibilityLabel(T("audio_output", "Audio output"))
                    }
            ...
            Section(T("selected_sounds", "Selected sounds")) {
                if !store.hasSelection {
                    ContentUnavailableView(T("choose_sounds", "Choose your sounds"), systemImage: "waveform")
                        .accessibilityIdentifier("player-empty-selection")
                }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                            ...
                            Text(L10n.soundLabel(sound.id))
                                .accessibilityIdentifier("player-sound-" + sound.id)
                            Spacer()
                            Button { store.unselect(sound.id) } label: {
                                Image(systemName: "minus.circle").frame(width: 44, height: 44).contentShape(Rectangle())
                            }
                            .accessibilityLabel(T("remove", "Remove") + ": " + L10n.soundLabel(sound.id))
                            .accessibilityIdentifier("remove-sound-" + sound.id)
